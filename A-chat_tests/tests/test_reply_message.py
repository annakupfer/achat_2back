...
def test_reply_message(access_token):
    ...
    try:
        ...
        # Проверяем parentId в reply
        parent_id = response_reply.get("messages", {}).get("parentId")
        assert parent_id == int(reply_payload["original_message_id"]), \
            f"Expected parentId {reply_payload['original_message_id']}, got {parent_id}"
